...
void RpcProvider::Run()
{
    ...
    // 绑定连接回调和消息读写回调方法    用muduo库的好处：很好的分离了网络代码和业务代码
    server.setConnectionCallback(std::bind(&RpcProvider::OnConnection, this, std::placeholders::_1)); // 设置连接回调
    server.setMessageCallback(std::bind(&RpcProvider::OnMessage, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));

    // 设置muduo库的线程数量
    server.setThreadNum(4); // 一个线程是I/O线程，剩下的三个是工作线程；这就是一个典型的基于reactor模型的一个服务器网络模型，epoll + 多线程

    // 把当前rpc节点上要发布的服务全部注册到zk上面，让rpc client可以从zk上发现服务
    // session tomeout   30s   zkclient 网络I/O线程，专门会以1/3 * timeout时间发送ping消息，作为一个心跳消息
    ZkClient zkCli;
    ...
    for (auto &sp : m_serviceMap)
    {
        ...
        {
            //  /service_name/method_name  /UserServerRpc/Login  存储当前这个rpc服务节点主机的ip和port
...
service_name   method_name     args      定义proto的message类型，进行数据头(service_name、method_name、args_size)的序列化和反序列化
                                            service_name  method_name  args_size
通过service_name找见ServiceInfo信息

16UserServiceLoginzhang san123456     (16就表示前16位表示数据头，然后剩下的就是参数数据)
header_size(4个字节) + header_str + args_str

但是在tcp通信时要考虑tcp的粘包问题，剩下的字符串有可能是当前的请求和下一个连续的请求的数据混到一块了，所以为了解决tcp粘包问题，我们得给args参数的长度也给记录一下

std::string  insert和copy方法   处理按内存大小处理数据非常有用
*/
